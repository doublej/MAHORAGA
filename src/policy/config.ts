...
export function validateOptionsPolicyConfig(config: unknown): OptionsPolicyConfig {
  ...
  if (
    typeof c.max_option_positions !== 'number' ||
    c.max_option_positions < 1 ||
    c.max_option_positions > 10
  ) {
...
export function validatePolicyConfig(config: unknown): PolicyConfig {
  ...
  if (
    typeof c.max_daily_loss_pct !== 'number' ||
    c.max_daily_loss_pct <= 0 ||
    c.max_daily_loss_pct > 1
  ) {
